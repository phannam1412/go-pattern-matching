...
func FullSearchAllKeywords(tokens []string, pos int) *Res {

	// Searching for all keyword-value pairs.
	expression := FullSearch(OnePair, -1)
	...
	if res.Children != nil {
		for _, item := range res.Children {
			keyword := item.Children[0].Value
			if isset(detected, keyword) {
				continue
			}
			detected[keyword] = item.Children[2].Value
		}
	}

	// Searching for email.
	if _, ok := detected["email"]; !ok {
		formula := FullSearch(Email, 1)
		res := formula(tokens, pos)
		if res != nil {
			detected["email"] = res.Value
		}
	}
	return &Res{
		Pos:  res.Pos,
		Expr: "full_search_all_keywords",
		Data: detected,
	}
}

func MatchLinkedIn(tokens []string, pos int) *Res {

	// Check if it's LinkedIn template.
	footer := And(
		Text("Contact"),
		OneTokenExceptLineBreak,
		OneTokenExceptLineBreak,
		OneTokenExceptLineBreak,
		Text("on"),
		OneTokenExceptLineBreak,
	...
	lookupFooter := LookupForOne(footer)
	res := lookupFooter(tokens, pos)
	if res == nil {
		return nil
	}
	...
	res = header(tokens, 0)
	pos = res.Pos
	fullname := res.Children[4].Value

	// Second line.
	secondLine := And(
		TextUntilEndAt(Text("at")),
		OneTokenExceptLineBreak,
		TextUntilLineEnd,
		)
	res = secondLine(tokens, pos)
	pos = res.Pos
	position := res.Children[0].Value
	workplace := res.Children[2].Value



	value := Res{
		Pos: pos,
		Expr: "linkedin_matching",
		Data: map[string]string{
			"fullname": fullname,
			"position": position,
			"workplace": workplace,
	...
	// Third line
	res = Email(tokens, pos + 1)
	if res != nil {
		value.Data["email"] = res.Value
	}

	return &value
}

func Main(tokens []string) map[string]string {
	main := Or(MatchLinkedIn, FullSearchAllKeywords)
	child := main(tokens, 0)
	if child == nil {
		return nil
	}
	return Normalize(child.Children[0].Data)
}

func Normalize(res map[string]string) map[string]string {

    if _, ok := res["email"]; ok {
        res["email"] = strings.ReplaceAll(res["email"], " ","")
		res["email"] = strings.ReplaceAll(res["email"], "\t","")

    }
    if _, ok := res["address"]; ok {
        res["address"] = strings.ReplaceAll(res["address"], " , ",", ")
        res["address"] = strings.ReplaceAll(res["address"], " . ",". ")
        res["address"] = strings.ReplaceAll(res["address"], " / ","/")
        res["address"] = strings.Trim(res["address"], ", ")
    }
    if _, ok := res["birthday"]; ok {
		res["birthday"] = strings.ReplaceAll(res["birthday"], "\t","")
		res["birthday"] = strings.ReplaceAll(res["birthday"], " / ","/")
		res["birthday"] = strings.ReplaceAll(res["birthday"], "/ ","/")
		res["birthday"] = strings.ReplaceAll(res["birthday"], "/ ","/")
		res["birthday"] = strings.ReplaceAll(res["birthday"], " , ",", ")
    }
    if _, ok := res["phone"]; ok {
        res["phone"] = strings.ReplaceAll(res["phone"], "( ","(")
        res["phone"] = strings.ReplaceAll(res["phone"], " )",")")
    }
